...
## 알려진 의도적 예외 (a11y — 버그 아님, 재플래그 금지)

- **분할 모드 비포커스 탭 디밍**: `.tabbar.split .tab-group:not(.active) .tab`가 `opacity: 0.55`라 비포커스 탭 제목이 ~2.3:1로 AA 미달. **의도적 de-emphasis** — 활성 탭이 배경+테두리+볼드+진한 잉크로 이미 포커스를 표시하고, 디밍을 약화하면 그 단서가 사라진다. 제품 오너 결정(2026-06-12)으로 **유지**. (frontend-design 리뷰에서 확인)

## 후속(상태색 텍스트) — 처리됨 2026-06-12

frontend-design 리뷰에서 상태색을 *텍스트*로 쓰던 곳(마감 위험·장시간 넛지·종료 버튼)이 색 시스템 규칙("상태=아이콘/칩") 위반 + AA 미달(2.7~4.2)로 확인 → 중립 잉크 + 틴트/dot으로 상태 전달하도록 수정, 종료 버튼은 `color-mix`로 테마별 AA 통과. 죽은 raw-hex 폴백 전부 제거. (커밋 `5d29f1e`)
