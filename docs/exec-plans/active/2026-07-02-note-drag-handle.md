...
- [ ] `src/note/DragHandle.ts` — Tiptap Extension 골격 + ProseMirror plugin
...
- [ ] `src/styles.css` — `.drag-handle` / `.drop-indicator` 스타일
- [ ] `src/note/NoteEditor.tsx` — `DragHandle` import 및 extensions 추가
- [ ] `make verify` 통과 확인
- [ ] `make dev`로 실제 동작 확인
