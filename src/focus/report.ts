...
/**
 * A markdown report of a period (for "export to note"). This is the ONE bridge
 * to evaluation (ADR 0007), so it leads with the "so what" (insights) and the
 * deadline-vs-focus table — the manager's first question — not just a session
 * dump. It's a plain .md the user reviews/edits before showing anyone.
 */
export function buildReportMarkdown(
  ...
  // insight lede — same "so what" the dashboard shows up top
  if (tips.length) {
    lines.push('## 한눈에');
    for (const t of tips) {
      const mark = t.tone === 'warn' ? '⚠️ ' : t.tone === 'good' ? '✅ ' : '· ';
      lines.push(`- ${mark}${t.text}`);
    }
    lines.push('');
  }

  // deadline vs actual focus — recent/upcoming deadlines only
  const prio = priority.filter((p) => p.dueInDays >= -7);
  if (prio.length) {
    lines.push('## 마감 vs 실제 집중');
    lines.push('| 주제 | 마감 | 실제 집중 |', '|---|---|---|');
    for (const p of prio.slice(0, 8)) {
      const due = p.dueInDays < 0 ? `${-p.dueInDays}일 지남` : p.dueInDays === 0 ? '오늘' : `D-${p.dueInDays}`;
      const flag = p.flagged ? ' ⚠️' : '';
      lines.push(`| ${p.label} | ${due}${flag} | ${p.focusSec > 0 ? fmtDuration(p.focusSec) : '0분'} |`);
    }
    lines.push('');
  }
