  ...
  },
  {
    ...
    body: (
      <>
        ...
        <K k="연결 포인트" d="스티키·이미지 상하좌우 4곳에 항상 흐리게 표시 — 선택하거나 hover하면 커지고 진해지며 클릭 가능해짐. 스티키 아래 쌓인 '텍스트 박스 추가' 블록도 각자 자기만의 연결 포인트를 가져, 카드 전체가 아니라 그 텍스트 블록 하나에 콕 집어 연결할 수 있음" />
        ...
        <K k="텍스트 박스 추가" d="카드 바깥쪽 아래에 작은 텍스트 블록이 쌓임 — 여러 개 추가 가능, hover 시 ×로 개별 삭제. 분리·독립 이동은 불가. 하나만 선택했을 때만 (스티키 전용). 각 블록에 화살표를 직접 연결할 수 있음 (위 '연결 포인트' 참조)" />
        ...
        <K k="⌘C / ⌘V" d="선택한 요소 복사 · 붙여넣기 — 양끝 모두 선택된 화살표도 함께 복사됨. 붙여넣기마다 원본에서 조금씩 떨어진 자리에 놓이고, 붙여넣은 요소들이 새 선택 상태가 됨" />
        <K k="⌘X" d="선택한 요소 잘라내기 — 복사 후 바로 삭제, ⌘V로 다시 붙여넣을 수 있음 (실행 취소 한 번으로 되돌아감)" />
        <K k="⌘A" d="현재 보드의 스티키 · 이미지 전체 선택 (화살표는 제외)" />
        <K k="우클릭" d="스티키 · 이미지 위에서 컨텍스트 메뉴 — 복제(오프셋 위치에 사본) · 삭제" />
