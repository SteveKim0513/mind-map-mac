  ...
  },
  {
    ...
    body: (
      <>
        ...
        <K k="정보" d="노트 제목 옆 — 목차 · 연결된 노드 · 백링크 · 보드에서 참조됨을 한곳에" />
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <K k="더블클릭" d="스티키노트 위에서는 내용 편집, 빈 캔버스에서는 그 자리에 새 스티키 생성" />
        <K k="편집 중 Escape" d="타이핑을 취소하고 편집 시작 전 텍스트로 되돌아감" />
        ...
        <K k="정리" d="하단 툴바 — 선택한 스티키와 화살표로 연결된(양방향) 모든 요소를 그래프 레이아웃 알고리즘으로 재배치해 화살표 교차를 최소화 — 여러 스티키가 서로 얽힌 관계도일수록 효과가 큼 (선택한 스티키 자신의 위치는 그대로)" />
        <div className="man-grp">화살표</div>
        <K k="연결 포인트" d="스티키·이미지 상하좌우 4곳에 항상 흐리게 표시 — 선택하거나 hover하면 커지고 진해지며 클릭 가능해짐" />
        ...
        <K k="색" d="화살표를 선택하면(여러 개도 가능) 뜨는 색상 버튼으로 스티키와 같은 팔레트에서 색 지정 — 화살촉도 항상 같은 색을 따라감" />
        <div className="man-grp">스티키/이미지 선택 메뉴 (위에 뜨는 플로팅 메뉴, 마인드맵 노드 선택과 동일한 방식 — 여러 개를 함께 선택하면(스티키끼리 또는 이미지끼리만) 색(+스티키는 모양·정렬·서식)이 한 번에 모두에 적용됨. 이미지를 선택하면 색·연동만 있는 같은 메뉴가 뜸)</div>
        <K k="색 변경" d="마인드맵 노드와 같은 팔레트 — 상단 태그바에서 색만 모아보기 필터 가능. 이미지는 배경을 채우는 대신 사진을 가리지 않도록 테두리에 아주 얇게 표시(기본값은 스티키와 동일한 노란색)" />
        <K k="모양 변경" d="사각형 · 타원 중 선택 — 하단 툴바에서 모양 필터 가능 (스티키 전용)" />
        <K k="정렬" d="가로(왼쪽 · 가운데 · 오른쪽) + 세로(위 · 중간 · 아래) (스티키 전용)" />
        <K k="글자 서식" d="크기(작게 · 중간 · 크게), 굵게 (스티키 전용)" />
        <K k="텍스트 박스 추가" d="카드 바깥쪽 아래에 작은 텍스트 블록이 쌓임 — 여러 개 추가 가능, hover 시 ×로 개별 삭제. 분리·독립 이동은 불가. 하나만 선택했을 때만 (스티키 전용)" />
        <K k="연동" d="스티키 또는 이미지 하나를 선택했을 때만 — 마인드맵 노드, 노트, 또는 외부 링크(URL)에 연결. 카드/사진 안(사진은 바로 아래)에 칩으로 표시되며 누르면 그 노드·노트로 이동하거나 링크를 새 창으로 열고, ×로 연결 해제. 연결된 노드·노트 쪽에도 '보드에서 참조됨' 칩이 뜨고, 누르면 이 보드로 돌아와 스티키·이미지를 바로 보여줌" />
        <K k="글자색" d="배경색에 맞춰 검정/흰색이 자동으로 정해짐" />
        <div className="man-grp">태그바 (경로 바 바로 아래)</div>
        <K k="색 필터" d="마인드맵과 같은 방식 — 칩을 누르면 그 색만 그리드로 가지런히 재배치해 보여주고, 나머지는 숨김(끄면 원래 자리로 복귀)" />
        ...
        <K k="⌘Z / ⌘⇧Z" d="실행 취소 · 다시 실행 — 이동·리사이즈는 드래그 한 번, 텍스트 편집은 한 번의 편집 세션 전체가 한 단계" />
      </>
    ),
  },
  {
