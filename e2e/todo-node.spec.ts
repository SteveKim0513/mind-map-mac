...
test('일반 노드 메뉴엔 "할 일로 전환"만, 전환하면 완료·일정·집중이 나온다', { tag: ['@todo', '@map'] }, async () => {
...
test('일정을 자연어로 잡으면 자동으로 할 일 노드가 된다', { tag: ['@todo', '@map'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('다중 선택에 일반 노드가 섞이면 "완료"가 안 보이고, 전원 할 일이면 보인다', { tag: ['@todo', '@map'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('"일반 노드로 되돌리기"는 할 일 상태를 지운다', { tag: ['@todo', '@map'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
