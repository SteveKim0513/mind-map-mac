...
test('⌘K가 활성 맵의 노드도 검색하고, 전체 검색으로 넘어갈 수 있다', { tag: ['@command', '@nav'] }, async () => {
  ...
  try {
    ...
    // 두 번째 노드를 만들고(Tab → 자식), 다중 선택 컨텍스트 메뉴가 열리고 항목이 나오는지 확인.
    // 축/속성 그룹 라벨은 제거됨(카피 감사 2026-07-15) — 구분선만으로 그룹을 나눈다.
    ...
    await expect(page.locator('.ctx-group-label')).toHaveCount(0);
    await expect(page.locator('.ctx-item', { hasText: '섹션으로 묶기' })).toBeVisible();
  } finally {
    await cleanup();
  }
});

test('탭이 5개 이상 열리면 화면 분할을 1회 제안한다', { tag: ['@command', '@nav'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    for (let i = 0; i < 5; i++) {
