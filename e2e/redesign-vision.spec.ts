...
test('설정의 고급 옵션은 기본으로 접혀 있고, 펼치면 정보 양식 링크가 보인다', { tag: ['@view', '@focus'] }, async () => {
...
test('노트 편집기 툴바는 "양식+" 버튼을 쓴다', { tag: ['@view', '@focus'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('집중 세션 위젯은 사이드바 독킹 시 기본으로 접혀 있고, 클릭하면 펼쳐진다', { tag: ['@view', '@focus'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('휴지통·노트 템플릿 배지는 숫자가 아닌 조용한 점으로 표시된다', { tag: ['@view', '@focus'] }, async () => {
