...
test('노드 텍스트에 "@내일 3시"를 커밋하면 자동으로 일정이 잡힌다', async () => {
...
test('"@" 없이 "오늘"을 라벨처럼 써도 일정으로 오인식하지 않는다', async () => {
...
test('노드 텍스트에 "#red"를 커밋하면 자동으로 색이 지정된다', async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
...
test('일반 텍스트("할 일이 3개 남았다")는 일정으로 오인식하지 않는다', async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
