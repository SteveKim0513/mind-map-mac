...
test('노트 템플릿 — 삽입, 사이드바 패널, 설정 토글로 숨기기', async () => {
  ...
  // 메타 템플릿도 하나 시딩 — 템플릿 노트에서는 "메타+"가 숨겨지는지 확인하기 위함.
  writeFileSync(
  ...
  try {
    ...
    // 없다 — "메타+" 버튼 자체가 보이면 안 된다.
    await expect(page.locator('button[title="메타 추가"]')).toHaveCount(0);

    // 일반 노트 탭으로 돌아가면 배지가 없고 노드 연동·메타+ 버튼은 그대로 있어야 한다(회귀 방지).
    ...
    await expect(page.locator('button[title="메타 추가"]')).toBeVisible();
