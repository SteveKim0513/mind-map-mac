...
test('새 노트 메뉴 명령(⌘⇧N 경로)이 노트 편집기를 연다', { tag: ['@command', '@note'] }, async () => {
...
test('⌘L로 선택한 노드에 노트 연결 피커가 열린다', { tag: ['@command', '@note'] }, async () => {
...
test('⌘K "미리알림 동기화 설정"으로 설정이 열리고 미리알림 행이 보인다', { tag: ['@command', '@note'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('빈 마인드맵에는 조용한 빈 상태 안내가 뜬다 (온보딩 코치 없음)', { tag: ['@command', '@note'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
