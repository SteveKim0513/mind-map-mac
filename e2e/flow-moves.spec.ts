...
test('첫 실행 코치가 빈 맵에 뜨고, 건너뛰면 사라지고, 다시 뜨지 않는다', async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
    // 처음 빈 워크스페이스 → 코치 등장
    await expect(page.locator('.frc')).toBeVisible({ timeout: 3_000 });
    await page.click('.frc-skip');
    await expect(page.locator('.frc')).toHaveCount(0);
    // 두 번째 빈 맵에는 다시 뜨지 않는다(1회성) — 대신 조용한 빈 상태
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
    await expect(page.locator('.frc')).toHaveCount(0);
