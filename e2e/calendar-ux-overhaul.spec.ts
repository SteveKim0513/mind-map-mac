...
test('일간 집중·완료 버튼이 둘 다 아이콘+라벨 pill로 통일됐다 (§3.1)', { tag: ['@calendar', '@schedule'] }, async () => {
...
test('스케줄 클릭 → 서브트리 미리보기 서랍 → "오른쪽에 열기"로 우측 분할 (§3.2)', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('일간 "일정 추가"에서 종일↔시간 토글이 동작한다 (§3.6)', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('"새로 만들기"는 오늘의 생각에 새 일정을 담는다 (§3.3)', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await newScheduledNode(page, '@오늘 오후 3시 시드노드');
    await openCalendar(page);
...
test('월간 셀은 3개를 넘겨도 "+N" 없이 전부 보여준다 (§3.7)', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('"일정 추가" 버튼이 일/주/월 헤더에 모두 있고, 토글 만진 뒤 Escape로 피커가 닫힌다', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await newScheduledNode(page, '@오늘 오후 3시 시드노드');
    await openCalendar(page);
...
test('주간 빈 슬롯으로 열면 피커에 토글이 보이고 "시간 지정"으로 프리필된다', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await newScheduledNode(page, '@오늘 오후 3시 시드노드');
    await openCalendar(page);
...
test('시간이 겹치는 일정은 주간 그리드에서 나란히 열 분할되고 hover로 확대된다', { tag: ['@calendar', '@schedule'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
