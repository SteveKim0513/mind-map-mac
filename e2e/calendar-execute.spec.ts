...
test('주간 시간 그리드에 시각 지정 노드가 블록으로 뜬다', async () => {
...
test('일간 요약이 오늘의 계획 개수를 보여준다 (계획↔실행)', async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('시각을 지정하면 SchedulePopover에 소요 시간 컨트롤이 뜨고, 칩 선택이 반영된다', async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('월간 뷰에서 일정 칩을 다음 날 셀로 드래그하면 그 날짜로 옮겨진다', async () => {
  const { page, cleanup } = await launchApp();
  try {
