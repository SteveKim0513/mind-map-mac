...
test('사이드바 "캘린더"를 누르면 캘린더가 탭으로 열리고 일간 뷰가 기본이다', { tag: ['@calendar'] }, async () => {
...
test('캘린더는 싱글턴 탭이다 — 두 번 열어도 탭이 하나만 생긴다', { tag: ['@calendar'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
...
test('오늘 일정이 있으면 일간 뷰에 나타나고, 집중을 시작할 수 있다', { tag: ['@calendar'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-section-btn[title="새 마인드맵"]');
    await page.waitForSelector('.canvas', { timeout: 5_000 });
...
test('일/주/월 토글이 각각 다른 레이아웃을 보여준다', { tag: ['@calendar'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-nav-item:has-text("캘린더")');
    await page.waitForSelector('.cal', { timeout: 5_000 });
...
test('월간 뷰에서 오늘 셀을 클릭하면 일간 뷰로 전환된다', { tag: ['@calendar'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
    await page.click('.sb-nav-item:has-text("캘린더")');
    await page.waitForSelector('.cal', { timeout: 5_000 });
...
test('⌘K 명령 팔레트의 "캘린더 열기"로도 캘린더 탭을 열 수 있다', { tag: ['@calendar'] }, async () => {
  const { page, cleanup } = await launchApp();
  try {
